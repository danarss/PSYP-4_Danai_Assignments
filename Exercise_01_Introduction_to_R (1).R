...
# By running the object name in the console (we can also call this **printing the contents of the object**), R will list all of it's contents.	
...
# You can **assign data to objects** using either the "<-" or "=". They are interchangable. Some programmers like to use these in different situations, for example assigning things with "<-" in case of a new object, and using "=" when an already existing object is changed, but this is up to you to decide.	
...
# You can use the **View()** function to check that the data file is really assigned to the mydata object.	
...
# Vectors can be coerced/forced to become another class or type.	
